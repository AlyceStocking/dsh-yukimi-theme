...
## 对话区现代化（2026-09-10）
- 对话记录：助手消息使用 20px 圆角、轻描边/阴影和 18×22px 留白；用户气泡采用不对称圆角；工具摘要保持紧凑并可展开。
- 提交区域：24px 圆角浮动输入框、浅/深色独立底色、聚焦光圈、36px 发送按钮和统一工具栏圆角；尊重减少动态效果设置。
- 使用宿主 data-* 和可访问性标记定位，无生成类名依赖；保留宿主输入、发送和菜单逻辑。
- 实际浏览器测试：多行输入后点击发送，收到“多行输入与发送正常”；提交后输入清空，空消息发送按钮禁用。
- 已验证工具过程展开/收起、模型菜单展开/关闭，以及浅色与深色外观；结束后恢复跟随系统。
- 390×844：输入框在视口内，document.scrollWidth=390，小人仍显示；结束后恢复默认视口。
- 最终计算样式：消息圆角20px、输入框圆角24px；语法检查、4/4单元测试及git diff --check通过。
- 已同步本机 DSH 安装副本。

## 小人选择框跟随修复（2026-09-10）
- 移除屏幕右下角固定坐标；选择框按小人位置展开，左侧空间不足时换到右侧，并限制在视口内。
- 浏览器验证：右侧小人菜单在左侧，相隔约8px、底部对齐；拖到左侧后菜单切换右侧。
- 菜单打开时继续拖动，小人和菜单同步移动；390×844下菜单边界为left78/right378/top12/bottom472，无越界。
- 通过键盘菜单键触发同一选择框逻辑；长按触发处理未变。恢复默认视口并刷新还原测试拖动位置。
- npm run check、npm test（4/4）、git diff --check通过；同步本机安装副本。

## Mobamas 默认关闭（2026-09-18）
- 初始化显示偏好改为 showMobamas=false，可在小人长按菜单中手动开启。
- 更新现有组件测试，验证默认隐藏、手动开启及窗口缩放后保留开启偏好。
- npm run check、npm test（4/4）、git diff --check通过；同步本机安装副本。
