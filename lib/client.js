	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function MobamasRail() {
				...
				const onDown = (e) => {
					const rect = rootRef.current ? rootRef.current.getBoundingClientRect() : { left: 20, top: 120 };
				...
				const rootStyle = pos === null
					? { position: "fixed", right: 20, top: 120, width: 196, pointerEvents: "auto", zIndex: 9999, fontFamily: "inherit" }
					: { position: "fixed", left: pos.x, top: pos.y, width: 196, pointerEvents: "auto", zIndex: 9999, fontFamily: "inherit" };
			...
			function YukimiPet() {
				...
				const onPetitPointerDown = (e) => {
					...
					const rect = rootRef.current ? rootRef.current.getBoundingClientRect() : { left: 24, top: 120 };
					dragStart.current = { sx: e.clientX, sy: e.clientY, bx: rect.left, by: rect.top, prevPos: pos };
				...
				const rootStyle = pos === null
					? { position: "fixed", left: 24, bottom: 24, zIndex: 9999, pointerEvents: "auto", fontFamily: "inherit" }
					: { position: "fixed", left: pos.x, top: pos.y, zIndex: 9999, pointerEvents: "auto", fontFamily: "inherit" };
				...
				slots.inject("shell.overlay", () => {
					slots.register(
						...
						() => React.createElement("img", {
							...
							style: {
								position: "fixed", right: 18, bottom: 14, width: 160,
								pointerEvents: "none", userSelect: "none", opacity: 0.9, zIndex: 9999,
					...
					slots.register(
						...
						() => React.createElement(MobamasRail),
					);
				});
			}
		}
