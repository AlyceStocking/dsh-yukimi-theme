window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			"body:has(.yukimi-dock:not([hidden])) #root { max-width: calc(100% - 248px); }",
		...
		function apply(ctx) {
			...
			if (theme !== undefined) {
				const tokens = {
					"--dsw-alias-bg-base": { light: "rgba(240,244,252,0.96)", dark: "rgba(13,16,38,0.96)" },
			...
			let preferences = { visible: true, showSign: true, showMobamas: true, showPet: true, bg: CARDS[23].url };
			...
			const toggleDecorations = () => update({ visible: !preferences.visible });
			...
			function DecorationDock({ usePreferences, update, toggleDecorations }) {
				...
				return React.createElement(React.Fragment, null,
					...
					React.createElement("aside", { id: "yukimi-dock", className: "yukimi-dock", hidden: !prefs.visible, "aria-label": "雪美装饰" },
						React.createElement("header", null, React.createElement("h2", null, "雪美 · Yukimi"),
							React.createElement("button", { onClick: toggleDecorations, "aria-label": "收起雪美装饰" }, "收起")),
